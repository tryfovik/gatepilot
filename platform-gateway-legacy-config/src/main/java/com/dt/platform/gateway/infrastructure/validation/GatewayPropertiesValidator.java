...
public class GatewayPropertiesValidator {
    ...
    private void validateAudit(GatewayProperties properties) {
        GatewayProperties.AuditProperties audit = properties.getAudit();
        if (!audit.isEnabled()) {
            return;
        }
        if (audit.getRecentCapacity() <= 0) {
            throw new IllegalArgumentException("gateway audit recent capacity must be positive");
        }
        requireText(audit.getTraceHeaderName(), "gateway audit trace header name must not be blank");
    }
    ...
    private void validateRouteGovernance(GatewayProperties properties) {
        for (var projectEntry : properties.getProjects().entrySet()) {
            ...
            for (var routeEntry : project.getRoutes().entrySet()) {
                ...
                validateCircuitBreaker(projectKey, routeKey, route);
    ...
    private void validateCircuitBreaker(String projectKey,
                                        String routeKey,
                                        GatewayProperties.RouteProperties route) {
        ...
        if (!route.isApiEnabled()) {
            throw new IllegalArgumentException("gateway circuit breaker requires api route to be enabled: "
                    + projectKey + "/" + routeKey);
        }
        if (!StringUtils.hasText(circuitBreaker.getName()) && circuitBreaker.getName() != null) {
            throw new IllegalArgumentException("gateway circuit breaker name must not be blank: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getStatusCodes() != null) {
            for (Integer statusCode : circuitBreaker.getStatusCodes()) {
                if (statusCode == null || HttpStatus.resolve(statusCode) == null) {
                    throw new IllegalArgumentException("gateway circuit breaker status code is unsupported: "
                            + projectKey + "/" + routeKey + " -> " + statusCode);
                }
            }
        }
        if (circuitBreaker.getFallbackUri() != null
                && !"forward".equalsIgnoreCase(circuitBreaker.getFallbackUri().getScheme())) {
            throw new IllegalArgumentException("gateway circuit breaker fallback uri only supports forward scheme: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getSlidingWindowSize() <= 0) {
            throw new IllegalArgumentException("gateway circuit breaker sliding window size must be positive: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getMinimumNumberOfCalls() <= 0
                || circuitBreaker.getMinimumNumberOfCalls() > circuitBreaker.getSlidingWindowSize()) {
            throw new IllegalArgumentException("gateway circuit breaker minimum calls must be between 1 and sliding window size: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getFailureRateThreshold() <= 0 || circuitBreaker.getFailureRateThreshold() > 100) {
            throw new IllegalArgumentException("gateway circuit breaker failure rate threshold must be between 0 and 100: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getWaitDurationInOpenState() == null
            ...
            throw new IllegalArgumentException("gateway circuit breaker open wait duration must be positive: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getPermittedNumberOfCallsInHalfOpenState() <= 0) {
            throw new IllegalArgumentException("gateway circuit breaker half-open permitted calls must be positive: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getSlowCallDurationThreshold() != null
                && (circuitBreaker.getSlowCallDurationThreshold().isZero()
                || circuitBreaker.getSlowCallDurationThreshold().isNegative())) {
            throw new IllegalArgumentException("gateway circuit breaker slow call duration threshold must be positive: "
                    + projectKey + "/" + routeKey);
        }
        if (circuitBreaker.getSlowCallRateThreshold() <= 0 || circuitBreaker.getSlowCallRateThreshold() > 100) {
            throw new IllegalArgumentException("gateway circuit breaker slow call rate threshold must be between 0 and 100: "
                    + projectKey + "/" + routeKey);
        }
        if (HttpStatus.resolve(circuitBreaker.getFallbackStatus()) == null) {
            throw new IllegalArgumentException("gateway circuit breaker fallback status is unsupported: "
                    + projectKey + "/" + routeKey + " -> " + circuitBreaker.getFallbackStatus());
        }
        if (circuitBreaker.getFallbackCode() < 0) {
            throw new IllegalArgumentException("gateway circuit breaker fallback code must not be negative: "
                    + projectKey + "/" + routeKey);
        }
        requireText(circuitBreaker.getFallbackMessage(), "gateway circuit breaker fallback message must not be blank: "
                + projectKey + "/" + routeKey);
        requireText(circuitBreaker.getContentType(), "gateway circuit breaker fallback content type must not be blank: "
                + projectKey + "/" + routeKey);
        try {
            MediaType.parseMediaType(circuitBreaker.getContentType());
        }
        catch (IllegalArgumentException exception) {
            throw new IllegalArgumentException("gateway circuit breaker fallback content type is invalid: "
                    + projectKey + "/" + routeKey + " -> " + circuitBreaker.getContentType(), exception);
        }
    }
    ...
    private void validateReleaseVariants(String projectKey,
        ...
        for (Map.Entry<String, GatewayProperties.ReleaseVariantProperties> entry : variants.entrySet()) {
            ...
            if (variant.getWeight() < 0 || variant.getWeight() > 100) {
                throw new IllegalArgumentException("gateway release variant weight must be between 0 and 100: "
                        + projectKey + "/" + routeKey + " -> " + entry.getKey());
            }
        ...
        if (totalWeight > 100) {
            throw new IllegalArgumentException("gateway release variant total weight must not exceed 100: "
                    + projectKey + "/" + routeKey);
        }
    }
