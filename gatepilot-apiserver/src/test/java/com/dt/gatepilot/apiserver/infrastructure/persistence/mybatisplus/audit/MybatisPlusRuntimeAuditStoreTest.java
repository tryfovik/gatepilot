...
class MybatisPlusRuntimeAuditStoreTest {
    ...
    void shouldPersistAndQueryRuntimeAudits() {
        ...
        store.saveBatch(List.of(
                record(namespace, "node-1", "trace-1", "route-a", "SUCCESS"),
                record(namespace, "node-1", "trace-2", "route-b", "LIMITED")
        ));

        CursorPage<RuntimeAuditRecord> page = store.list(query(namespace, "route-a", 10));
