...
参考 Helm values 的配置体验和 Kubernetes Dashboard 的资源预览，但页面风格仍然保持 GatePilot Console 的运维控制台风格。

必须包含：

- 中文表单填写项目、路由、上游、认证、限流、熔断、重试、染色、灰度和蓝绿参数。
- 环境 profile，例如测试、预发、生产。
- values 预览。
- 渲染后的 GatePilot 声明式资源预览。
- Kubernetes 部署清单预览。
- dry-run 校验结果。
- diff 结果。
- 提交前确认。
...
- 页面只生成配置草稿、发布请求或部署清单，不直接改 proxy / agent。
- 模板文案、默认值和校验规则要集中管理，不能散落在页面组件里。
- 复杂配置要分步骤填写，避免一个大表单无限堆字段。
